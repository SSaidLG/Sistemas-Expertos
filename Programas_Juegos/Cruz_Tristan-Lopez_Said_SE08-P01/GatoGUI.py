...
# Este programa implementa el Juego del Gato utilizando una interfaz gráfica
# construida con Tkinter. La lógica de la Inteligencia Artificial se basa en el
# algoritmo Minimax optimizado con Poda Alfa-Beta, el cual evalúa el árbol de 
# posibles movimientos para asegurar que la IA siempre tome la decisión óptima,
# minimizando la posibilidad de perder y buscando ganar o forzar un empate.
# =============================================================================
...
def verificarGanador(tablero, jugador):
    # Revisa todas las filas, columnas y las dos diagonales.
    # Devuelve True si el 'jugador' (X u O) ha ocupado una línea completa.
    for fila in tablero:
...
def movimientosDisponibles(tablero):
    # Recorre la matriz de 3x3 y devuelve una lista de coordenadas (fila, columna)
    # correspondientes a las casillas que aún están vacías (" ").
    return [(f, c) for f in range(3) for c in range(3) if tablero[f][c] == " "]

def minimax(tablero, esMaximizando, alfa, beta):
    # Algoritmo recursivo que evalúa todos los posibles futuros del tablero.
    # Asigna +10 si gana O, -10 si gana X, y 0 en caso de empate.
    # Utiliza la poda Alfa-Beta para descartar ramas innecesarias y ser más eficiente.
    ...
    if esMaximizando:
        ...
        for f, c in movimientosDisponibles(tablero):
            ...
            tablero[f][c] = " " # Deshace el movimiento temporal
            alfa = max(alfa, mejor)
            if beta <= alfa:
                break # Poda la rama
        return mejor
...
def mejorMovimientoIA(tablero):
    # Inicia la búsqueda Minimax asumiendo que la IA juega como "O" (maximizador).
    # Evalúa el primer nivel de movimientos posibles y devuelve la mejor coordenada.
    mejorPuntaje = -1000
    movimiento = None
    for f, c in movimientosDisponibles(tablero):
        tablero[f][c] = "O"
...
class GatoApp:
    # Paleta de colores para la interfaz (estilo oscuro/moderno)
    COLORES = {
        "fondo":    "#1e1e2e",
        "panel":    "#2a2a3e",
        "boton":    "#313145",
    ...
    def _construirUI(self):
        # Construye todos los elementos visuales: Títulos, marcador, etiquetas de estado,
        # la cuadrícula de botones de 3x3 y los controles inferiores (radios y botón de reinicio).
        ...
        # Creación de los 9 botones de la cuadrícula
        for f in range(3):
            fila_botones = []
            for c in range(3):
                ...
                # Eventos para cambiar el color del botón al pasar el mouse por encima
        ...
        # Selector de turno (Radiobuttons para elegir jugar como X o como O)
        ...
        # Botón para reiniciar la partida manualmente
    ...
    def nuevaPartida(self):
        # Reinicia el tablero lógico (matriz) y el tablero visual (botones).
        # Define quién empieza según la elección del jugador.
        ...
        # Limpia visualmente los botones
        for f in range(3):
            for c in range(3):
        ...
        # Si el jugador eligió O, la IA (que ahora es X) empieza automáticamente
    ...
    def jugadaHumano(self, fila, col):
        # Procesa el clic del usuario en una casilla.
        # Valida que el juego esté activo y la casilla vacía.
        ...
        # Verifica si el humano ganó (casi imposible con esta IA) o si hay empate
    ...
    def _mejorMovIA_X(self):
        ...
        # Función auxiliar de búsqueda para cuando el humano decide jugar con 'O'
        mejorPuntaje = 1000
        movimiento = None
    ...
    def _colocarMarca(self, fila, col, marca):
        # Actualiza el estado lógico de la matriz y desactiva el botón presionado
        # pintando la 'X' o la 'O' con su color correspondiente.
    ...
    def _resaltarGanador(self, jugador):
        # Busca cuál fue la línea (fila, columna o diagonal) que conectó el ganador
        # y cambia el color de fondo de esos tres botones para destacarlos visualmente.
        for f in range(3):
    ...
    def _finJuego(self, resultado):
        # Detiene la partida, actualiza el puntaje correspondiente e informa
        # el resultado final en la etiqueta de estado.
        self.juegoActivo = False
    ...
    def _actualizarPuntaje(self):
        # Refresca el texto del marcador superior con las victorias y empates actuales.
...
if __name__ == "__main__":
    # Inicializa la raíz de Tkinter y lanza el bucle principal de la aplicación.
    root = tk.Tk()
    app = GatoApp(root)
    root.mainloop()
